...
test('TC-16 Check с пустым ответом не запускает проверку', async ({ page }) => {
  ...
  // #score удалён; проверяем что #progress-info не изменился (нет «изучено»)
  await expect(page.locator('#progress-info')).toContainText('предложений');
});
...
// TC-18: правильный ответ → #progress-info обновляется (показывает «изучено»)
test('TC-18 После правильного ответа #progress-info показывает счётчик изученных', async ({ page }) => {
  await page.goto('/0.html');
  await buildCorrectAnswer(page);
  await page.locator('#btn-check').click();
...
test('TC-25 Reset очищает localStorage и перезапускает игру', async ({ page }) => {
  ...
  // После сброса #progress-info снова показывает полное количество предложений
  await expect(page.locator('#progress-info')).toContainText('предложений');
});
...
// ==============================================================
for (const n of ['0', '1', '2', '3']) {
  test(`TC-34-${n} Кнопка «Назад» существует на ${n}.html и ведёт на index.html`, async ({ page }) => {
    await page.goto(`/${n}.html`);
...
// ==============================================================
// TC-35: #progress-info виден и показывает количество предложений
// ==============================================================
const EXPECTED_INITIAL = { '0': '196', '1': '196', '2': '203', '3': '197' };
for (const n of ['0', '1', '2', '3']) {
  test(`TC-35-${n} #progress-info виден и содержит ${EXPECTED_INITIAL[n]} на ${n}.html`, async ({ page }) => {
    await page.goto(`/${n}.html`);
    await expect(page.locator('#progress-info')).toBeVisible();
    await expect(page.locator('#progress-info')).toContainText(EXPECTED_INITIAL[n]);
  });
}

// ==============================================================
// TC-36: после правильного ответа #progress-info обновляется
// ==============================================================
test('TC-36 После правильного ответа #progress-info показывает «X / N изучено»', async ({ page }) => {
  await page.goto('/0.html');
  await buildCorrectAnswer(page);
  await page.locator('#btn-check').click();
  const text = await page.locator('#progress-info').textContent();
  expect(text).toMatch(/\d+ \/ \d+ изучено/);
});

// ==============================================================
// TC-37: чипы имеют увеличенный шрифт (≥ 18px)
// ==============================================================
test('TC-37 Чипы в word-bank имеют font-size ≥ 18px (крупный шрифт)', async ({ page }) => {
  await page.goto('/0.html');
  const fontSize = await page.locator('#word-bank .chip').first().evaluate(
