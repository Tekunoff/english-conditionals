...
test('TC-30 Прогресс сохраняется после перезагрузки страницы', async ({ page }) => {
  ...
  expect(scoreAfter).toContain('✓ 1');

  // Перезагрузить страницу
  await page.reload();

  // Счётчик изученных должен сохраниться
  const scoreReloaded = await page.locator('#score').textContent();
  expect(scoreReloaded).toContain('✓ 1');
});
...
test('TC-32 importProgress восстанавливает прогресс из JSON', async ({ page }) => {
  ...
  // После импорта score должен показывать 5 изученных (done.size = 5)
